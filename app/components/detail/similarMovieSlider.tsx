...
export function SimilarMovies({ Id, Type }: SimilarMoviesProps) {
  ...
  useEffect(() => {
    const loadMovies = async () => {
      try {
        let data;
        if (Type === "movies") {
          data = await fetchSimilarMovieDetail(Id, "1");
        } else if (Type === "series") {
          data = await fetchSimilarTvDetail(Id, "1");
        }
        setMovies(data);
      } catch (err) {
